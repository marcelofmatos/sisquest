...
class metabase_ibase_class extends metabase_database_class
{
	...
	Function SkipFirstRows($result)
	{
		...
		{
			if(GetType(@ibase_fetch_row($result))!="array")
			{
	...
	Function FetchRow($result,$row)
	{
		...
		{
			if(GetType($this->results[$result_value][$this->current_row[$result_value]+1]=@ibase_fetch_row($result))!="array")
			{
	...
	Function EndOfResult($result)
	{
		...
		if(GetType($this->row_buffer[$result_value]=@ibase_fetch_row($result))=="array")
			return(0);
		Unset($this->row_buffer[$result_value]);
		$this->rows[$result_value]=$this->current_row[$result_value]+1;
		return(1);
